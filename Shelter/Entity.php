...
abstract class Entity implements IEntity {
	 ...
	 * todo není to čisté, je to pouze pro použítí v mapperu, checkeru apod., ale zvenku by to být nemělo...
	 * možnosti: mít friend visibility jen pro mapper apod., ta ale v php není
	 *      předávat celý array originalů od entity->save() přes mapper do checkeru... ale to je zbytečně komplikovaný a nehezký
	 *      mít to zkátka public... co se dá dělat.. reset(), isChanged() atd. jsou taky o private paramech a public...
	 */
	 ...
	 * @return bool
	 */
	protected function hasLazy($param)
	{
		return FALSE;
	}
	 ...
	 * @return mixed
	 */
	protected function lazyLoad($param)
	{
	}
		...
		if (!$this->hasUnwrapper($param, $unwrapper)) {
			if (!$hasClear) {
				...
				if ($this->hasWrapper($param)) {
					throw new MemberAccessException(get_class($this)  . ": Cannot write to a read-only parameter $param.", self::WRITE_READONLY);
				}

				// undeclared param
				throw new MemberAccessException(get_class($this)  . ": Cannot write to an undeclared parameter $param.", self::WRITE_UNDECLARED);
			}
			...
			try {
				...
				if ($value === NULL && $assigned) {
					throw new \UnexpectedValueException(
						...
						. "If new value is NULL, throw Nais\\Entity\\NullValueException instead."
					);
				}
			}
